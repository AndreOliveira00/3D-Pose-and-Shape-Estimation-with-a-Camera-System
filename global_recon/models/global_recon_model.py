...
class GlobalReconOptimizer:
    ...
    def load_model(self):
        if 'motion_traj_cfg' in self.specs:
            self.mt_cfg = MotionTrajConfig(self.specs['motion_traj_cfg'])               # Inicilização do STAGE 2 -> Generative Motion Infiller
            self.mt_model = MotionTrajJointModel(self.mt_cfg, self.device, self.log)    # Inicilização do STAGE 3 -> Global Trajectory Predictor
        else:
    ...
    def init_data(self, in_dict):
        ...
        cam_pose = torch.eye(4).repeat((num_fr, 1, 1)).float().to(self.device)  # Inicialização da posição da camara ao longo de todos os frames (matriz 4x4)
        ...
        if self.est_type == 'hybrik':
            for idx, pose_dict in in_dict['est'].items():
                ...
                new_dict['smpl_pose'] = smpl_pose_wroot[:, 1:].reshape(-1, 69)  # Remove root joint das matrizes com angulos das juntas do modelo SMPL (inicialmente (frames,24,3, após [:, 1:]) fica (frames,23,3),
                if idx in in_dict['gt']:                                        #   logo, smpl_pose_wroot[0][1] = smpl_pose_wroot[:, 1:][0][0]
    ...
    def init_cam_pose(self, data, all_frames=False):
        ...
        for pose_dict in data['person_data'].values(): 
            cam_pose_inv_new.append(torch.matmul(pose_dict['person_transform_world'], pose_dict['person2cam']) * pose_dict['vis_frames'][:, None, None])    # Posição da camara 
    ...
    def forward(self, data, opt_variables, opt_meta):
            ...
            if 'cam' in opt_variables:
                ...
                for i in range(len(num_persons)):
                    if num_persons[i] == 0:
                        ...
                        last_cam = data['cam_pose_inv'][i]
                cam_rot_6d = rotmat_to_rot6d(data['cam_pose_inv'][:, :3, :3])   # cam_rot_6d.shape = [frames,6]
                cam_rot_6d[num_persons == 0] += data['cam_inv_rot_residual']       
                data['cam_pose_inv'][:, :3, :3] = rot6d_to_rotmat(cam_rot_6d)       # Extrinsecos? -> tensor [frames,3,3]
                ...
                data['cam_pose'] = inverse_transform(data['cam_pose_inv'])      # Extrinsecos completos? tensor [frames,4,4]
        ...
        for pose_dict in data['person_data'].values():
            ...
            if 'smpl_pose' in pose_dict and 'cam_K' in pose_dict:
                ...
                joint_3d = smpl_motion.joints   # joint_3d.shape = joint_3d_cam.shape = [frames,3,3]        # joint_3d -> X ->  computed using the SMPL joint function J from the optimized global pose 
                cam_pose = data['cam_pose'] # ATENÇÃO AQUI!!!!! -> VER QUAL É A MATRIZ DOS EXTRINSECOS -> cam_pose??? -> tensor [frames,4,4]
                joint_3d_cam = transform_trans(cam_pose, joint_3d)  # Do mundo para o sistema de coordenandas da camara -> coordenadas da pessoa face ao referencial da camara
                pose_dict['kp_2d_pred'] = perspective_projection(joint_3d_cam, pose_dict['cam_K'])      # Loss function E2D (projeção 2D a partir de pontos 3D (referencial da camara) e intrinsecos)
                # pose_dict['kp_2d_pred'].shape = [388, 26, 2]
                if self.flag_use_pen_loss:
    ...
    def optimize(self, in_dict, continue_opt=False):
        data = tensor_to(in_dict, self.device) if continue_opt else self.init_data(in_dict)     # Inicialização camera e poses (smpl parameters)
