...
class Visualizer3D:
    ...
    def __init__(self, init_T=6, enable_shadow=False, anti_aliasing=True, use_floor=True,
        ...
        if platform.system() == 'Linux':
            print('Attention: Your OS (Linux) may not support anti-aliasing, but you can try to turn in on in lib.utils.visualizer3d.__init__() to improve rendering quality.')
            anti_aliasing = False
    ...
    def init_scene(self, init_args):
        if not self.hide_env:
            ...
            self.pl.set_background('#FCC2EB', top='#C9DFFF')    # Classic Rose -> Lavender Blue (background atrás do cenário)
        # shadow
        ...
        if self.use_floor:
            ...
            tex = pyvista.numpy_to_texture(make_checker_board_texture('#81C6EB', '#D4F1F7'))    # Desenhar chão xadrez
    ...
    def setup_key_callback(self):
        ...
        previous frame: Left\n \
        next frame: Right\n \
        rint camera focus and position: t\n \
        go to frame: z\n \
        toggle smpl mesh: j\n \
    ...
    def show_animation(self, window_size=(800, 800), init_args=None, enable_shadow=None, frame_mode='fps', fps=30, repeat=True, show_axes=True):
        self.interactive = True
    ...
    def save_animation_as_video(self, video_path, init_args=None, window_size=(800, 800), enable_shadow=None, fps=30, crf=25, frame_dir=None, cleanup=True):
        ...
        print("Preparing optimized frames...")
        ...
        shutil.copytree(frame_dir, f'{os.path.dirname(os.path.dirname(video_path))}/grecon_images/{time.strftime("%Y%m%d-%H%M%S")}')
        if cleanup:
            shutil.rmtree(frame_dir)
